...
func TestGetTxByID(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			if tc.Name == "empty txID in req" {
				idVal = ""
			}
			req := httptest.NewRequest("GET", fmt.Sprintf("/transactions/%v", idVal), nil)
	...
	for _, tc := range txErrTests {
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			mockSvc.On("GetTransactionByID", mock.Anything, tc.UserID, tc.TxID).Return((*models.Tx)(nil), tc.TxErr)
