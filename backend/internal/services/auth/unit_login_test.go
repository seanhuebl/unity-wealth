...
func TestLogin(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.deviceFound {
				dummyQueries.On("GetDeviceInfoByUser", ctx.Request.Context(), mock.Anything).Return(validDeviceID, nil)
				dummyQueries.On("RevokeToken", ctx.Request.Context(), mock.Anything).Return(nil)
			} else {
				dummyQueries.On("CreateDeviceInfo", ctx.Request.Context(), mock.Anything).Return(validDeviceID, nil)
			}
